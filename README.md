...
Selective Worker-first routing covers the root, canonical trailing-slash page URLs, and the exact `/mcp` protocol endpoint. Direct Markdown, Pagefind, JavaScript, CSS, fonts, images, and well-known artifacts remain on the free static-asset path. Canonical page and MCP requests count toward the Workers request allowance, and Cloudflare does not fall back to static delivery after that allowance is exhausted. Monitor Worker invocations. If the account plan exposes per-product usage notifications, configure a Workers request threshold that alerts the team well before the Free plan's 100,000-request daily limit. Otherwise use external monitoring, or move the project to Workers Paid before traffic can approach that limit. If neither is acceptable, remove or redesign content negotiation rather than accepting Cloudflare Error 1027 responses across Worker-routed page requests.
