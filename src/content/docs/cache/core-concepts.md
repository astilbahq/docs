...
- A **negative entry** remembers an opted-in 404 without inventing a value. An invalidation-fresh negative resolves as <code>undefined</code>; it is never served through grace or stale-on-error. Before writing one, Cache checks the same L2 Store and refuses when that read finds a protected value. Its declared duration is not enforced yet.
