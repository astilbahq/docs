...
| `recipe.version` | Version of that recipe's output contract. All recipes in Create 0.1.0 use version 1. |
...
`ownership.structured` divides a JSON file into independently tracked fields. In 0.1.0, that file is `package.json`.
...
Create 0.1.0 does not include an update, migration, or `doctor` command. Today the manifest helps you:
