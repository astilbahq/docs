...
test("presents products and copies the agent setup prompt from the homepage", async ({
  ...
  await expect(envProduct).toContainText("@astilba/env 0.2.2");
...
test("presents Env as a distinct public-alpha product home", async ({
  ...
  await expect(
    page.getByText(
      "Released as @astilba/env 0.2.2 for public-alpha evaluation. Expect deliberate breaking changes before a stable release."
    )
  ).toBeVisible();
  await expect(
